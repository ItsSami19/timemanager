...
export default function UserStatisticsOpenedPage({ params }: Props) {
  // Entpacken der params mit use() (da params jetzt ein Promise ist)
  const { id } = React.use(params); // ← Verwende use() um das Promise zu entpacken
  ...
  return (
    <Box sx={{ p: 4, bgcolor: theme.palette.background.default }}>
      ...
      <Typography variant="h4" gutterBottom sx={{ fontWeight: 'bold', color: theme.palette.text.primary }}>
        User Statistics for ID: {id}
      </Typography>

      <Divider sx={{ mb: 4 }} />

      {/* Tables for time account and vacation data */}
              ...
              <TableBody>
                {vacationData.map((entry) => (
                  <React.Fragment key={entry.year}>
                    ...
                    {/* Weitere Zeilen für Requested, Approved, etc. */}
                  </React.Fragment>
                ))}
              </TableBody>
            </Table>
