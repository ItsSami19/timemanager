        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right" sx={{ fontWeight: 'bold', color: theme.palette.text.primary }}>See Statistics</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filteredEmployees.filter(emp => emp.role !== 'HR').map((emp) => (
              <TableRow key={emp.id}>
                <TableCell sx={{ color: theme.palette.text.primary }}>{emp.id}</TableCell>
                <TableCell sx={{ color: theme.palette.text.primary }}>{emp.name}</TableCell>
                <TableCell sx={{ color: theme.palette.text.primary }}>{emp.role}</TableCell>
                <TableCell align="right">
                  <Button
                    variant="contained"
                    color="success"
                    onClick={() => router.push(`/dashboard/hr/hrstat/${emp.id}/hruserstatopen`)} // ✅ Korrekte Route // !!!route wichtig hier !!!!
                  >
                    Open Statistics
                  </Button>
                </TableCell>
            ...
            {filteredEmployees.filter(emp => emp.role !== 'HR').length === 0 && (
              <TableRow>
